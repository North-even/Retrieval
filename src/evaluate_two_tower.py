...
def build_user_histories(train_df, max_history_len):
    ...
    for user, history in user_histories.items():
        ...
        padded_histories[int(user)] = history
...
def evaluate():
    ...
    with torch.no_grad():
        all_item_vecs = model.encode_item(
            item=all_item_ids,
    ...
    with torch.no_grad():
        for _, row in tqdm(valid_df.iterrows(), total=len(valid_df), desc="Evaluating"):
            ...
            scores = torch.matmul(user_vec, all_item_vecs.T).squeeze(0)
